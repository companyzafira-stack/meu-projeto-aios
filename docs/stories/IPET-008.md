...
## Acceptance Criteria
- [x] Página "Agenda" com duas seções: Configuração + Visualização
- [x] **Configuração de Horários:** para cada dia da semana (seg-sáb), definir:
  - Horário de início (ex: 08:00)
  - Horário de fim (ex: 18:00)
  - Toggle ativo/inativo (ex: domingo = inativo)
- [x] **Configuração de Slots:**
  - Duração do slot: dropdown (30min, 45min, 60min, 90min)
  - Atendimentos simultâneos: input numérico (1-5)
- [x] **Bloqueio de Horários:** botão "Bloquear Horário" para bloquear data/horário específico
...
- [x] **Visualização Calendário:** semana atual com slots ocupados vs disponíveis
  - Cores: verde (disponível), azul (agendado), vermelho (bloqueado)
  - Clique no slot mostra detalhes do agendamento (se houver)
- [x] **Regra de Inatividade:** se pet shop não atualizar slots por 7 dias, campo "última atualização" fica amarelo. Se 14 dias, pet shop fica hidden no app
...
## Testing
- [x] Configurar horário segunda-sábado funciona
- [x] Domingo desativado não gera slots
- [x] Duração de slot gera intervalos corretos (60min = 8:00, 9:00, 10:00...)
- [x] Atendimentos simultâneos permite N bookings no mesmo slot
- [x] Bloquear horário marca slot como indisponível
- [x] Bloqueio com agendamento existente mostra aviso
- [x] Calendário visual mostra cores corretas
- [x] Function get_available_slots retorna slots corretos
